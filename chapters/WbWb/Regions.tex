...
\subsection{Unfolding procedure and cross secrtion definition}
...
The definition of the differential fiducial cross section as a function of an observable $X$ in a given bin $i$ is given by the formula
\begin{equation}
    \frac{d\sigma_\text{fid}}{dX_i} = \frac{1}{L\cdot \Delta X_i} \cdot \frac{1}{\epsilon_i} \cdot \sum_j M_{ij}^{-1}f_{\text{acc}}^j(N_{d,j}-N_{b,j}).
    \label{eq:Unfolding}
\end{equation}
The measured quantity $N_{d,j}$ denotes the number of data events in bin $j$.
The first step subtracts the number of background events $N_{b,j}$.
These are obtained from data driven methods or MC simulations, as explained in Section~\ref{}. The remaining events are considered as signal events which is defined by \ttbar and $tWb$.



The response matrix $M_R$ keeps tracks of event migration.
Particle level events generated in bin $i$ that end up in bin $j$ after the detector simulation are filled in $M_{R,ij}$. An ideal measurement would therefore produce a perfectly diagonal matrix. 
Due to the limited resolution in real-world experiments, off-diagonal elements may also be filled. The migration matrix $M_{ij}$ is obtained when normalizing the response matrix
\begin{equation}
...
Therefore, a \textit{regularization} term is added which ensures a dampening of this effect and therefore a smooth unfolded distribution.
This analysis employs the Tikhonov regularization scheme~\cite{Tikhonov1995} is employed as implemented in the TUnfold package~\cite{Schmitt:2019um}.
...
$f^j_\text{acc} = \sum_i M_{R,i,j}$.

The complementary efficiency $\epsilon_i = \sum_j M_{R,i,j}^\text{norm}$ 
is defined as the fraction of matched events out of all events passing the particle-level selection. It corrects for events that fail to be reconstructed or selected at the detector level. This can result from limitations in detector coverage, insufficient resolution, trigger thresholds or reconstruction algorithms.
