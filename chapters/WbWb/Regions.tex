...
\begin{equation}
    \frac{d\sigma_\text{fid}}{dX_i} = \frac{1}{L\cdot \Delta X_i} \cdot \frac{1}{\epsilon_i} \cdot \sum_j M_{i,j}^{-1}f_{\text{acc}}^j(N_{d,j}-N_{b,j}).
    \label{eq:Unfolding}
\end{equation}
...
These are obtained from data driven methods or MC simulations, as explained in Section~\ref{sec:bkg-estimation}. The remaining events are considered as signal events which is defined by \ttbar and $tWb$.
...
$f^j_\text{acc} = \sum_i M_{R,i,j}^\text{norm}$.
